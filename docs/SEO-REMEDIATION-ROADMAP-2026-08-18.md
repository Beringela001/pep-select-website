...
| M3 — Indexation and schema | Crawl paths, pagination, GSC inspection, connected Product/Offer/Organization/Dataset schema | Deployed and verified on Live in child theme `0.25.0-beta.26` and COA Archive `0.7.2`. Invalid Shop pagination returns 404; Offer seller and Dataset creator share the existing organization ID; Dataset publication date is present; both schema emitters use the same context. GSC outcome checks remain pending. | Live gate passed on 2026-08-18: named backup, route/schema/email regression, cache purge, and Paulo approval. GSC URL Inspection/recrawl remains the outcome gate. |
