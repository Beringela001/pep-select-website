...
| Background work | Kinsta's 24-hour window reports 450 PHP thread-limit events, 6,238 PHP requests, 0.84-second average PHP/MySQL response time, and no PHP memory-limit events. The Action Scheduler queue runner was the slowest recurring path at about 5.62 seconds average. | Requests can queue during traffic or cron bursts even without a broad 500 pattern. | Monitor after this maintenance window. The dominant remaining overlap is Action Scheduler plus FluentCRM/admin activity, so continue with the email-delivery pass before considering more workers. |
| Scheduled Actions | The 134 failures remain historical WooPayments/migration records. All 24 pending actions were legitimate future recurring or one-time work owned by WooCommerce, FluentCRM, WP Mail SMTP, Action Scheduler, and Back In Stock Notifier. Live totals remained 134 failed and 24 pending after deployment. | Bulk deletion would remove useful history without reducing current work. | No cleanup performed. Recheck only if actions become overdue or the failed count increases. |
| Updates and vulnerability | Kinsta identifies Elementor Pro 3.34.0 as the vulnerable component and offers 4.2.2. Elementor's licensed download endpoint returned `Unauthorized` on both staging and Live even after the dashboard reported the license Active. | The security update remains blocked by the vendor license/package service. Updating Elementor Core alone would leave an unvalidated mixed Core/Pro pair. | Repair the Elementor account/license download authorization, then update Pro 4.2.2 and Core 4.2.3 together on staging before Live. Core remains 4.1.5 and Pro remains 3.34.0 for compatibility. |
...
| Pinterest | The owner confirmed Pinterest is not used. The integration was already inactive. | Unused code and invalid-token noise. | Removed Pinterest for WooCommerce from staging and Live. |
