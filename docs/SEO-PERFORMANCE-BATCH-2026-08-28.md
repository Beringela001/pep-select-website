...
**Release candidate:** `0.25.0-beta.59`
...
checkout, payment, rewards, COA, access-gate, analytics, or advertising behavior.
...
| PERF-13: AdSense chain | Medium | Site Kit injects AdSense on every sampled storefront template; Lighthouse attributes 254,718 bytes and 543.9 ms main-thread time on the slow desktop-home run. | External Site Kit configuration. | Needs Paulo approval | Removing or delaying ads changes advertising behavior and possibly revenue; no silent change is permitted. |
...
- Stop before changing Site Kit analytics or AdSense until Paulo confirms the
  intended storefront advertising policy.

## Rollback

Revert release candidate `0.25.0-beta.59`; the original PNG assets remain in
