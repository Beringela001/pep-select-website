( function ( window, document, $ ) {
	...
	var autocompleteSnapshot = null;
	var autocompleteSnapshotExpires = 0;
	var autocompleteRefreshTimer = null;
	var addressFieldNames = [ 'address_1', 'address_2', 'city', 'state', 'postcode', 'country' ];
	...
	function captureAutocompleteAddress( prefix ) {
		var values = {};
	...
	function syncSelectDisplay( field ) {
		if ( $ && field && field.tagName === 'SELECT' ) {
			$( field ).trigger( 'change.select2' );
		}
	}

	function restoreAutocompleteAddress() {
		var changed = false;
		var sameAsShipping;

		if ( ! autocompleteSnapshot || Date.now() > autocompleteSnapshotExpires ) {
			autocompleteSnapshot = null;
			return false;
		}

		addressFieldNames.forEach( function ( name ) {
			var field = document.getElementById( autocompleteSnapshot.prefix + '_' + name );
			var value = autocompleteSnapshot.values[ name ];

			if ( field && field.value !== value ) {
				field.value = value;
				syncSelectDisplay( field );
				changed = true;
			}
		} );

		sameAsShipping = document.getElementById( 'billing_same_as_shipping' );
		if ( autocompleteSnapshot.prefix === 'shipping' && sameAsShipping && sameAsShipping.checked ) {
			addressFieldNames.forEach( function ( name ) {
				var billing = document.getElementById( 'billing_' + name );
				var value = autocompleteSnapshot.values[ name ];

				if ( billing && billing.value !== value ) {
					billing.value = value;
					syncSelectDisplay( billing );
					changed = true;
				}
			} );
		}
	...
	function queueCompleteAddressRefresh() {
		if ( ! $ ) {
			return;
		}

		window.clearTimeout( autocompleteRefreshTimer );
		autocompleteRefreshTimer = window.setTimeout( function () {
			restoreAutocompleteAddress();
			$( document.body ).trigger( 'update_checkout' );
		}, 80 );
	}

	function stabilizeAutocompleteAddress( event ) {
		var input = event.target;
		var prefix = /^billing_/.test( input.id || '' ) ? 'billing' : 'shipping';

		// Fluid Checkout may already have an AJAX refresh in flight from a state
	...
	document.addEventListener( 'fc_gaa_address_autocompleted', stabilizeAutocompleteAddress );

	if ( $ ) {
		$( document.body ).on( 'updated_checkout', function () {
			ensurePuertoRicoAutocompleteCountryOption();
			if ( restoreAutocompleteAddress() ) {
				queueCompleteAddressRefresh();
			}
			queueWarningUpdate();
		} );
