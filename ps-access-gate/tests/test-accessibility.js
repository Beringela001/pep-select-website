...
assert.match(plugin, /\* Version:\s+2\.2\.2/);
assert.match(readme, /^Version: 2\.2\.2$/m);
...
assert.ok(plugin.includes("main.focus({ preventScroll: true })"));
assert.doesNotMatch(plugin, /main\.focus\(\);/);
