...
Pep Select is the lightweight presentation child theme for the controlled customer-facing rebuild. Version 0.25.0-beta.3 completes SEO Milestone 5, removes the empty noindexed-post sitemap from Yoast's public sitemap index, and updates the homepage batch-matching education example to the documented Tesamorelin 10 mg batch `PSTES1071926GX`. WooCommerce, COA, checkout, account, rewards, verification, shipping, and order behavior remain unchanged.
