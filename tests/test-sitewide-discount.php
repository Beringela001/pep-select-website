...
function apply_filters( $hook, $value, ...$args ) {
	if ( 'pepselect_sitewide_discount_product_eligible' === $hook ) {
		return in_array( (int) $args[0], array( 11, 22 ), true );
	}
	return $value;
}
...
pepselect_assert( '0.00' === $rule['threshold_amount'], 'no-minimum rules ignore a supplied amount' );
...
pepselect_assert( 20.0 === PepSelect_Sitewide_Discount::estimated_discount_amount( $cart, $rule ), 'discount estimate excludes accessories' );

$rule['threshold_type'] = 'subtotal';
$rule['threshold_amount'] = '101';
pepselect_assert( ! PepSelect_Sitewide_Discount::cart_qualifies( $cart, $rule ), 'compound subtotal minimum excludes accessories' );
...
pepselect_assert( array( 11 ) === $coupon['product_ids'], 'coupon is restricted to compounds currently in cart' );
