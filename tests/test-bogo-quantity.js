...
assert.match(plugin, /PEPSELECT_BOGO_VERSION', '2\.3\.1'/);
...
assert.match(sitewideClass, /SCHEMA_VERSION\s*= 3/);
assert.match(sitewideClass, /allowed_automatic_discounts/);
assert.match(sitewideClass, /sitewide_takeover'\s*=>\s*'always_exclusive'/);
assert.match(sitewideClass, /shipping_policy'\s*=>\s*'unchanged'/);
assert.doesNotMatch(sitewideClass, /Allow every discount/);
...
assert.match(stackingClass, /candidate_for_code/);
